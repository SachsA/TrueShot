...
public:
    ...
    void sendInput(const InputState& in) {
        ...
        BitWriter bw;
        bw.writePOD((uint8_t)PacketType::ClientInput);
        bw.writePOD(in);
        ENetPacket* pkt =
            enet_packet_create(bw.buf.data(), bw.buf.size(), ENET_PACKET_FLAG_RELIABLE);
    ...
    void onEvent(ENetEvent& ev) {
        ...
        case ENET_EVENT_TYPE_RECEIVE: {
            ...
            if (t == (uint8_t)PacketType::Snapshot) {
                ...
                Snapshot s;
                if (!br.readPOD(s.tick)) break;
                uint32_t n;
                if (!br.readPOD(n)) break;
                s.entities.resize(n);
                for (uint32_t i = 0; i < n; i++) {
                    br.readPOD(s.entities[i].id);
                    br.readPOD(s.entities[i].pos.x);
                    br.readPOD(s.entities[i].pos.y);
                    br.readPOD(s.entities[i].pos.z);
                    br.readPOD(s.entities[i].vel.x);
                    br.readPOD(s.entities[i].vel.y);
                    br.readPOD(s.entities[i].vel.z);
                    br.readPOD(s.entities[i].yaw);
                    br.readPOD(s.entities[i].pitch);
                }
