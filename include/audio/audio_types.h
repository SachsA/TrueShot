...
struct AudioListener {
    ...
    std::vector<float> categoryVolumes{1.0f, 1.0f, 1.0f, 1.0f,
                                       1.0f, 1.0f, 1.0f, 1.0f}; // Per-category volumes
};
...
struct AudioZone {
    ...
    bool contains(const glm::vec3& point) const {
        ...
        return point.x >= minBounds.x && point.x <= maxBounds.x && point.y >= minBounds.y &&
               point.y <= maxBounds.y && point.z >= minBounds.z && point.z <= maxBounds.z;
    }
};
